        ...
        <>
            <header
                className={`fixed z-50 transition-all duration-500 ease-in-out border-b border-white/10 ${isScrolled
                    ? "top-4 left-1/2 -translate-x-1/2 w-[95%] sm:w-[90%] md:max-w-5xl rounded-lg bg-white/60 backdrop-blur-xl shadow-2xl border-white/20"
                    : "top-0 left-0 w-full bg-white/60 backdrop-blur-md border-gray-100"
                    }`}
            >
                <div className={`mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-500 ${isScrolled ? "h-16" : "h-20"}`}>
                    <div className="flex justify-between items-center h-full">
                        ...
                        <div className="flex-shrink-0 flex items-center cursor-pointer gap-2">
                            <img src="/logo.png" alt="Campus Connect Logo" className={`object-contain transition-all duration-500 ${isScrolled ? "h-32" : "h-40"}`} />
                        </div>
                        ...
                        <nav className="hidden md:flex items-center space-x-1">
                            {['Home', 'About', 'Contact'].map((item) => (
                                <Link
                                    ...
                                    className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-blue-600 hover:bg-blue-50/50 transition-all duration-200"
                                >
                                    {item}
                                </Link>
                            ))}
                    ...
                    <div className="absolute top-full left-0 w-full p-4 md:hidden">
                        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden animate-in slide-in-from-top-2">
                            <nav className="flex flex-col p-2">
                                ...
                                <Link
                                    to="#"
                                    className="text-center text-sm font-medium text-gray-600 hover:text-gray-900 py-3 hover:bg-gray-50 rounded-xl mx-2"
                                    onClick={() => setIsMenuOpen(false)}
                                >
                                    Sign In
                                </Link>
